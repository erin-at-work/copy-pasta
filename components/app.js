...
const App = ({ user: { id: userId, entries = [] } }) => {
  ...
  const handleOnSubmit = async ev => {
    ...
    try {
      const resp = await postNewEntry({ body, userId });
            ...
            <section>
              ...
              <div className="pt-4">
                {["github", "stripe", "emojis", "docker", "react", "js"].map(
                  tag => {
                    return (
                      <span key={tag} className="mr-2 mb-2 inline-block bg-gray-200 rounded-full px-3 py-1 text-sm text-gray-600">
                        #{tag}
                      </span>
                    );
                  }
